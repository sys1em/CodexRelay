...
每个类别使用主页从上到下的统一故障顺序，自定义 API 与已导入的二狗子 Profile 可以相互拖动。通用设置可选择“手动提示”或“自动切换”，分别控制 401、403、5XX、连接异常、目录失效和目录消失，并可调整认证次数、异常次数、统计窗口及列表末尾是否循环。自动模式下可为单个 Profile 设置“跳过此令牌”；手动模式仍允许选择该项。自动切换在本次运行的一轮故障中只尝试每个 Profile 一次，全部失败后停止；当前活动令牌重新请求成功后会清理故障提示并允许开始新一轮。
...
余额、套餐、公告和各 API 类别的令牌切换使用独立的右下角窗口，不依赖主窗口是否可见。同一提醒类型的多条内容合并显示：余额按账户 ID、套餐按套餐 ID 把确认状态写入 `config.json`，恢复到阈值以上后清理；套餐过期或进入 24 小时到期窗口时仅在剩余额度高于提醒阈值的情况下提示。令牌状态或分组恢复后，仍在本地的恢复项也会提示；自动切换已经离开原令牌时，弹窗以当前活动项为切换前置。绑定阶段只建立当前提醒基线，不弹出历史额度提醒，后续启动和同步按保存状态继续判断。
